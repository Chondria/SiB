...
As you may have observed in `src/mock.rs`, each pallet's `Config` trait was implemented for `Test` and all relevant `Config` types were defined for each pallet.
